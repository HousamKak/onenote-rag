...
class ImageStorageService:
    ...
    async def clear_all(self) -> int:
        ...
        This is used when clearing the entire index/database.
        ...
        if self.storage_type == "local":
            try:
                # Recursively delete all files in the base path
                ...
                for item in sorted(self.base_path.rglob('*'), reverse=True):
                ...
                logger.info(f"Cleared all images from local storage: {deleted_count} files deleted")
                    ...
                    if 'Contents' in page:
                        for obj in page['Contents']:
                            try:
                                ...
                                logger.error(f"Error deleting S3 object {obj['Key']}: {str(e)}")
 
                logger.info(f"Cleared all images from S3 storage: {deleted_count} files deleted")
 
            except Exception as e:
                logger.error(f"Error clearing S3 storage: {str(e)}")
